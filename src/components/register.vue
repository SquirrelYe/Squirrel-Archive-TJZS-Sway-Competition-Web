        ...
        <div class="panel-body">
          <div class="form-horizontal m-t-20">
            ...
            <div class="form-group text-center m-t-40">
              <div class="col-xs-12">
                <button
                  class="btn btn-primary waves-effect waves-light btn-lg w-lg"
                  @click="register()"
                >
                  注册
                </button>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本协议为您与本商业沙盘系统的管理者（天津多维汇教育咨询有限公司）之间所订立的契约，具有合同的法律效力，请您仔细阅读。
            </p>
            <hr />
            <h4>协议内容</h4>
            <p>
              ...
              只要您使用本系统平台服务，则本协议即对您产生约束，届时您不应以未阅读本协议的内容或者未获得本系统对您问询的解答等理由，主张本协议无效，或要求撤销本协议。您确认：本协议条款是处理双方权利义务的契约，始终有效，法律另有强制性规定或双方另有特别约定的，依其规定。
              您承诺接受并遵守本协议的约定。如果您不同意本协议的约定，您应立即停止注册程序或停止使用本系统平台服务。本系统有权根据需要不定期地制订、修改本协议及/或各类规则，并在本系统平台公示，不再另行单独通知用户。变更后的协议和规则一经在网站公布，立即生效。如您不同意相关变更，应当立即停止使用本系统平台服务。您继续使用本系统平台服务的，即表明您接受修订后的协议和规则。
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (this.check) {
          if (this.judge == this.code) {
            ...
            req
              .post(
                `api/sway?judge=4&name=${this.name}&pass=${this.pass}&cname=${this.cname}&email=${this.email}`
              )
              .then((res) => {
    ...
    judgeName() {
      ...
      if (this.name != "") {
        req
          .post(`api/sway?judge=1&name=${that.name}`)
          .then((res) => {
    ...
    judgeFirst() {
      var that = this;
      req.post(`api/sway?judge=2&email=${that.email}`).then((res) => {
        var jf = JSON.stringify(res.data.success);
        if (jf.indexOf("true") != -1) {
    ...
    sendcode() {
      ...
      s_alert.basic("发送中……");
      req
        .post(`api/mail?judge=0&mail_address=${this.email}&code=${num}`)
        .then((res) => {
