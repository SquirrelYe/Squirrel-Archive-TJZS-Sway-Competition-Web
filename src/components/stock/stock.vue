        ...
        <div class="modal-content">
            ...
            <div class="modal-content p-0">
              <ul class="nav nav-tabs navtab-bg nav-justified">
                <li class="active">
                  <a href="#profile-2" data-toggle="tab" aria-expanded="true">
                    ...
                    <span class="hidden-xs" style="font-size: 23px"
                      >普通产品上架</span
                    >
                  </a>
                </li>
              ...
              <div class="tab-content">
                <div
                  ...
                  id="profile-2"
                  v-if="currentShowGoodItem"
                >
                  <address
                    class="ng-scope"
                    align="center"
                    v-if="currentShowGoodItem.commerresearch"
                  >
                    <strong>产品名称:</strong
                    >{{ currentShowGoodItem.commerresearch.name }}<br />
                    <strong>产品单价:</strong
                    >{{ currentShowGoodItem.commerresearch.price }}<br />
                    <strong>产品配方:</strong><br />
                    <strong>
                      金：{{
                        currentShowGoodItem.commerresearch.s1 | sumSource(1)
                      }}
                      木：{{
                        currentShowGoodItem.commerresearch.s2 | sumSource(1)
                      }}
                      水：{{
                        currentShowGoodItem.commerresearch.s3 | sumSource(1)
                      }}
                      火：{{
                        currentShowGoodItem.commerresearch.s4 | sumSource(1)
                      }}
                      土：{{
                        currentShowGoodItem.commerresearch.s5 | sumSource(1)
                      }} </strong
                    ><br />
                    <strong>产品库存:</strong>{{ currentShowGoodItem.sum
                    }}<br />
                    <strong style="color: red"
                      >最高单价:{{ maxPrice }}万元</strong
                    ><br />
                  </address>
                </div>
                <div class="panel-body">
          ...
          <div class="modal-footer">
            ...
            <button
              ...
              data-dismiss="modal"
              @click="toPublic(currentShowGoodItem)"
              v-if="currentShowGoodItem.sum > 0"
            >
              产品上架
            </button>
          </div>
          ...
          <div class="modal-body" align="center">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-10">
                          <select class="form-control" v-model="oemCompany">
                            <option
                              v-for="(item, index) in showOemCompany"
                              :key="index"
                              :value="item.id"
                            >
                              {{ item.name }}
            ...
            <div class="modal-footer">
              ...
              <button
                ...
                v-if="
                  number > 0 &&
                  number <= currentOemItem.sum &&
                  oemCompany != company_id
                "
              >
                定向发送产品
              </button>
...
export default {
  ...
  methods: {
    ...
    showMyCompete() {
      req
        .post_Param("api/ass/industryyield_commerresearch", {
          judge: 3,
    ...
    chooseGoodItem(item) {
      ...
      let sum = 0,
        total = 0;
      if (item.commerresearch.s1 != 0) {
        sum++;
        total += item.commerresearch.s1;
      }
      if (item.commerresearch.s2 != 0) {
        sum++;
        total += item.commerresearch.s2;
      }
      if (item.commerresearch.s3 != 0) {
        sum++;
        total += item.commerresearch.s3;
      }
      if (item.commerresearch.s4 != 0) {
        sum++;
        total += item.commerresearch.s4;
      }
      if (item.commerresearch.s5 != 0) {
        sum++;
        total += item.commerresearch.s5;
      }
      // 获取商业用地品牌提升
      req
    ...
    updateStock(results, number) {
      ...
      // 写入交易
      req.post(
        `api/transaction?judge=1&id=0&Yearid=${this.Yearid}&inout=1&type=1&kind=2&price=0&number=${number}&me=${this.company_id}&other=${results[0][0].company_id}&commerresearch_id=${results[0][0].commerresearch_id}`
      );
      this.init();
