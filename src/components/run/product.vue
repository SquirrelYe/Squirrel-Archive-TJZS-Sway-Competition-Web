              ...
              <ul class="nav tabs-vertical">
                ...
                >
                  <a
                    ...
                    data-toggle="tab"
                    aria-expanded="false"
                    @click="chooseInduslandIndex(index)"
                    >工业用地 I{{ item.id }}</a
                  >
                </li>
              </ul>
                  ...
                  <ul class="nav tabs-vertical">
                    <li class="" v-for="(item, index) in tFactory" :key="index">
                      <a
                        ...
                        data-toggle="tab"
                        aria-expanded="false"
                        @click="chooseFactoryIndex(index)"
                        >工厂信息 F{{ item.id }}</a
                      >
                    </li>
                  </ul>
                  <div class="tab-content">
                    ...
                    >
                            ...
                            >
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td>
                                    {{ currentFactoryItem.includeline }}条
                                  </td>
                                  <td>{{ currentFactoryItem.price }}万</td>
                                  <td>
                                    {{
                                      currentFactoryItem.indusland_factory
                                        .number
                                    }}
                                  </td>
                                </tr>
                      ...
                      <div class="tab-pane">
                        ...
                        <div
                          class="tab-content"
                          v-for="(item, index) in currentLineItem.lines"
                          :key="index"
                        >
                          <div class="panel panel-default">
                            <div class="panel-heading">
                              ...
                              <strong>价值折旧率:</strong>{{ item.relief
                              }}<br />
                              <strong>良品率:</strong>{{ item.yield }}<br />
                              <strong>价值:</strong>{{ item.price }}<br />
                              <strong>建设要求:</strong
                              >{{ item.conrequire | formatConquire }}<br />
                              <strong>已有数量:</strong
                              >{{ item.indusland_factory_line.number }}<br />
                              <hr />
                              <button
                                ...
                                v-if="
                                  item.indusland_factory_line.condition == 1
                                "
                              >
                              ...
                              <button
                                ...
                                @click="openSetting(item)"
                                v-if="
                                  item.indusland_factory_line.condition == 0
                                "
                              >
                              ...
                              <button
                                ...
                                @click="openSetting(item)"
                                v-if="
                                  item.indusland_factory_line.condition == 2
                                "
                              >
            ...
            <div class="modal-body" align="center">
              <div class="row">
                <div class="col-lg-4">
                  <div class="portlet">
                    <div class="portlet-heading bg-inverse">
                      <h3 class="portlet-title">工业用地</h3>
                    ...
                    <div id="bg-inverse" class="panel-collapse collapse in">
                      <div class="portlet-body">
                        ...
                        <strong>效率提升:</strong>{{ tIndusland.efficient
                        }}<br />
                        <strong>折旧降低:</strong>{{ tIndusland.repurchase
                        }}<br />
                        <hr />
                      </div>
                    </div>
                ...
                <div class="col-lg-4">
                  <div class="portlet">
                    ...
                    <div id="bg-primary" class="panel-collapse collapse in">
                      <div class="portlet-body">
                        <strong>工厂编号:</strong>{{ currentFactoryItem.id
                        }}<br />
                        <strong>工地型号:</strong>{{ currentFactoryItem.model
                        }}<br />
                        <strong>占用面积:</strong>{{ currentFactoryItem.measure
                        }}<br />
                        <strong>可容纳生产线:</strong
                        >{{ currentFactoryItem.includeline }}<br />
                        <hr />
                      </div>
                    </div>
                  </div>
                ...
                <div class="col-lg-12" v-if="tLine.indusland_factory_line">
                  <div
                    class="portlet"
                    v-if="tLine.indusland_factory_line.condition == 0"
                  >
                    <div class="portlet-heading portlet-default">
                      <h3 class="portlet-title text-dark">选择要生产的产品</h3>
                      <div class="portlet-widgets">
                        <a href="javascript:;" data-toggle="reload"
                          ><i class="ion-refresh"></i
                        ></a>
                        ...
                        <div class="tab-content">
                          <div class="tab-pane active" id="me">
                            ...
                            >
                              ...
                              <tbody>
                                <tr
                                  v-for="(item, index) in showGoodItems"
                                  :key="index"
                                ...
                                >
                                  <td @click="getResearch(1, item)">
                                    <div
                                      class="radio radio-success radio-single"
                                    >
                                      <input
                                        type="radio"
                                        name="sigle"
                          ...
                          <div class="tab-pane" id="oem">
                            ...
                            >
                              ...
                              <tbody>
                                <tr
                                  v-for="(item, index) in showOEMItems"
                                  :key="index"
                                  :class="{ active: index == 1 }"
                                >
              ...
              <div v-if="tLine.indusland_factory_line">
                <div v-if="tLine.indusland_factory_line.condition == 0">
                  <strong>产品名称:</strong
                  ><strong>{{ chooseResearch.name }}</strong
                  ><br />
                  <strong>预计产量（单位/量）:</strong
                  ><strong>{{ number | sumCreat(tLine.yield) }}</strong
                  ><br />
                  <strong>预计结束时间:</strong
                  ><strong>{{
                    number | predict(tLine.capacity, currentLineNumber)
                  }}</strong
                  ><br />
                  <div v-if="chooseResearch != ''">
                    <strong
                      >需要耗费元素: 金：{{
                        chooseResearch.s1 | sumSource(this.number)
                      }}
                      木：{{
                        chooseResearch.s2 | sumSource(this.number)
                      }}
                      水：{{
                        chooseResearch.s3 | sumSource(this.number)
                      }}
                      火：{{
                        chooseResearch.s4 | sumSource(this.number)
                      }}
                  ...
                  <strong style="color: green"
                    >当前库存元素储量为：金_{{ showHaveTotalSource[0] }}、木_{{
                      showHaveTotalSource[1]
                    }}、水_{{ showHaveTotalSource[2] }}、火_{{
                      showHaveTotalSource[3]
                    }}、土_{{ showHaveTotalSource[4] }}、</strong
                  >
                  <br />
                  <strong>请输入需要生产的产品数量:</strong
                  ><input
                    type="number"
                    :disabled="this.judgeInput"
                    v-model="number"
                    @input="judgeTotalSurce()"
                  /><strong>（单位/件）</strong><br />
                </div>
                <!-- 生产中 -->
                <div v-if="tLine.indusland_factory_line.condition == 1">
                  <!-- <strong>产品名称:</strong><strong>{{chooseResearch.name}}</strong><br> -->
                  <h4 style="color: #00b300">生产中</h4>
                  <br />
                  <strong>预计结束时间:</strong
                  ><strong>{{
                    tLine.indusland_factory_line.start
                      | predictEnd(tLine.indusland_factory_line.stay)
                  }}</strong
                  ><br />
                </div>
                <!-- 生产完成 -->
                <div v-if="tLine.indusland_factory_line.condition == 2">
                  <strong>产品名称:</strong
                  ><strong>{{ FinishedResearchItem.name }}</strong
                  ><br />
                  <h4 style="color: #00b300">生产完成</h4>
                  <strong>产量（单位/量）:</strong
                  ><strong>{{
                    tLine.indusland_factory_line.stay
                      | iScreated(
                        tLine.capacity,
                        tLine.indusland_factory_line.number
                      )
                  }}</strong
                  ><br />
                  <strong
                    >耗费元素: 金：{{
                      FinishedResearchItem.s1
                        | iScreatedSource(
                          tLine.indusland_factory_line.stay,
                          tLine.capacity,
                          tLine.indusland_factory_line.number
            ...
            <div class="modal-footer">
              <div v-if="tLine.indusland_factory_line">
                ...
                <button
                  ...
                  "
                  v-if="
                    tLine.indusland_factory_line.condition == 0 &&
                    number != '' &&
                    number > 0 &&
                    judge &&
                    chooseResearch != ''
                ...
                <button
                  ...
                  class="btn btn-primary waves-effect waves-light"
                  data-dismiss="modal"
                  @click="reBuild(FinishedResearchItem)"
                  v-if="tLine.indusland_factory_line.condition == 2"
                >
                  存入库存
                </button>
              </div>
  ...
  methods: {
    ...
    openSetting(item) {
      ...
      if (this.tLine.indusland_factory_line.condition == 2) {
        this.showFinishedResearchItem(
          item.indusland_factory_line.commerresearch_id
        );
      }
      // 清空radio状态
    ...
    getShowGoodItems() {
      ...
      apis.getOneGoodByCompanyId(this.company_id).then((res) => {
    ...
    //显示开采的原料  用于生产产品
    getTotalSource() {
      apis.getOneSourceStockByCompanyId(this.company_id).then((res) => {
        print.log(res.data);
        let s1 = 0,
          s2 = 0,
          s3 = 0,
    ...
    judgeTotalSurce() {
      let judge = false;
      if (
        this.chooseResearch.s1 * this.number <= this.showHaveTotalSource[0] &&
        this.chooseResearch.s2 * this.number <= this.showHaveTotalSource[1] &&
        this.chooseResearch.s3 * this.number <= this.showHaveTotalSource[2] &&
        this.chooseResearch.s4 * this.number <= this.showHaveTotalSource[3] &&
        this.chooseResearch.s5 * this.number <= this.showHaveTotalSource[4]
      ) {
        this.judge = true;
      } else {
        this.judge = false;
    ...
    // 点击生产
    sendPrice(stay, capacity, number) {
      //研发产品信息，持续时间，产能，生产线数量
      ...
      // 更新每一个原料数量
      let usedSource = [
        researchitem.s1 * this.number,
        researchitem.s2 * this.number,
        researchitem.s3 * this.number,
        researchitem.s4 * this.number,
        researchitem.s5 * this.number,
      ];
      // 循环更改数量
    ...
    chooseFactoryIndex(index) {
      ...
      this.currentFactoryItem =
        this.showCompeteIndusland[this.induslandIndex].factories[
          this.factoryIndex
        ];
    ...
    getInduslandFactoryHaveLineItem() {
      req
        .post_Param("api/ass/indusland_factory_line", {
          judge: 8,
          indusland_factory_id:
            this.showCompeteIndusland[this.induslandIndex].factories[
              this.factoryIndex
            ].indusland_factory.id,
        })
        .then((res) => {
    ...
    forToChangeCondition(data) {
      ...
      for (let s = 0; s < data.length; s++) {
        ...
        if (Number(data[s].indusland_factory_line.condition) == 1) {
          ...
          let endTime = moment(data[s].indusland_factory_line.start)
            .add(data[s].indusland_factory_line.stay, "minutes")
            .format("YYYY-MM-DD HH:mm:ss");
    ...
    reBuild(FinishedResearchItem) {
      ...
      print.log(
        "加入库存，重新生产->",
        FinishedResearchItem,
        this.tLine.indusland_factory_line.commerresearch_id
      );
      let sum =
        this.tLine.indusland_factory_line.stay *
        this.tLine.capacity *
        this.tLine.indusland_factory_line.number;
      req
