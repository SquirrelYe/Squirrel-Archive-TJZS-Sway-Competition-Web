...
module.exports = {
  ...
  updateOneCompeteConditionById(id, condition) {
    return req.post_Param("api/compete", {
      judge: 2,
      id: id,
      condition: condition,
    });
  ...
  getOneStatisticByCompanyId(company_id) {
    return req.post_Param("/api/ass/company_statistic", {
      judge: 5,
  ...
  // 库存（产品&原料）
  getAllGoodsStock() {
    return req.post_Param("/api/ass/industryyield_commerresearch", {
      judge: 4,
    });
  },
  ...
  getOneSourceStockByCompanyId(company_id) {
    return req.post_Param("/api/miniyield", {
      judge: 4,
      company_id: company_id,
    });
  },
  ...
  getOneGoodByCompanyId(company_id) {
    return req.post_Param("/api/commerresearch", {
      judge: 5,
      company_id: company_id,
    });
  },
  ...
  getOneTransationByCompanyId(company_id) {
    return req.post_Param("/api/transaction", {
      judge: 8,
      company_id: company_id,
    });
  },
